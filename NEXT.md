...
`main` is at `0.21.0-dev` with a merged security change that is **released nowhere and running nowhere**. The `SEESEE_FORWARDED_ALLOW_IPS` default narrowing (see Just Completed) only protects a deployment that is actually running it, so this outranks any feature work below.

> **The live deployment is much further behind than "not yet deployed" implies.** `https://seesee.bpf.fyi` (Coolify app `kkss44kko80w48gs4c8kgc4o`) reports **`0.19.16-dev`** at `/api/v1/health` — it predates 0.20.0 entirely. The app is pinned to `ghcr.io/brandonjp/seesee-email:latest` with build pack `dockerimage`, and Coolify does **not** auto-pull on a new registry push, so every image built since early July has been sitting unpulled. The redeploy is a manual button, not a missing pipeline; GHCR has published every commit successfully (`0.20.0`, `0.20`, `latest`, and per-sha tags through `6b6c244`).
>
> **Consequence for step 1: do NOT delete the legacy key columns in this cut.** The Known Issue below says `_resolve_legacy_fallback`, the `resolve_smtp_password` fallback branch, and the `apps.api_key` / `apps.smtp_password` columns should go "in 0.21.0" — but that assumed 0.20.0 had been running in production long enough that no 0.19.x container could come back. It never ran in production at all. Deploying 0.21.0 takes the live instance straight from `0.19.16-dev` (pre-schema-v4) to `0.21.0`, which is *exactly* the upgrade path the fallback and the v4 backfill exist to survive. Cut 0.21.0 with all three still in place, confirm the live keys still authenticate over REST and SMTP after the jump, and defer the deletion to a later version once 0.21.0 has actually been running.
...
2. **Redeploy on Coolify** (`docs/src/content/docs/guides/coolify-deployment.md`). The app already exists and is healthy — this is a manual **Redeploy** on `kkss44kko80w48gs4c8kgc4o` to pull the new image, not a fresh setup. Consider changing the image tag from `latest` to `0.21.0` so what is running is identifiable from the Coolify UI. Confirm `SEESEE_BASE_URL` is `https://seesee.bpf.fyi` — with the narrowed default it is the safety net that keeps cookies `Secure`, and it cannot be read over the read-only Coolify API, so check it by hand. Limits are already capped at 256m / 0.5 CPU; leave them unless the v4 migration needs headroom. **Do not preemptively set `SEESEE_FORWARDED_ALLOW_IPS`** — Coolify's proxy is on a private Docker network and is already covered; setting it by hand would mask a real failure.
3. **Verify in production, with evidence.** `curl https://seesee.bpf.fyi/api/v1/health` should report `0.21.0` (it reports `0.19.16-dev` today — that is the check that the redeploy actually took). Confirm the schema v4 migration ran cleanly on the live pre-v4 database and that an existing app key still authenticates over **both** REST and SMTP. Log in over HTTPS and confirm the session cookie carries `Secure`; confirm the startup `SEESEE_BASE_URL` warning is absent; confirm the access log shows a real client IP, not the proxy's internal address. Production is the first real test of the decision record's claim that these ranges cover Coolify — if anything surprises you, add a postscript to `docs/decisions/2026-07-27-forwarded-allow-ips-default.md`.
...
- **Legacy key columns must be deleted — but NOT in 0.21.0.** `_resolve_legacy_fallback` in `seesee/keys.py`, the matching fallback branch at the end of `resolve_smtp_password`, and the legacy `apps.api_key` / `apps.smtp_password` columns exist only to survive a 0.19.x↔0.20.0 deploy overlap. They are commented "delete in 0.21.0" in three places — **that comment is now wrong and the code must stay.** The precondition was "0.20.0 has been running long enough that no 0.19.x container can come back," and 0.20.0 never ran in production: the live instance is still `0.19.16-dev`, so the 0.21.0 deploy *is* the 0.19.x→v4 upgrade the fallback was written for. Revisit once 0.21.0 has actually been running; then remove all three plus the `revoke_key` tombstone write and drop the columns in a schema v5 migration. Until then a revoked *primary* app key stays revoked only because `revoke_key` tombstones those columns — do not remove that write in isolation. Update the three "delete in 0.21.0" comments to point at the new target version when the cut happens.
