...
### Changed
- **`uvicorn[standard]` floor raised from `>=0.30.0` to `>=0.31.0`.** CIDR notation in `forwarded_allow_ips` only works from 0.31; 0.30.x compares trusted hosts as plain strings, so the new private-range default would have matched nothing and silently reintroduced the insecure-cookie bug it exists to prevent (v0.20.3-dev)
