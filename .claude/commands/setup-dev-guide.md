# SeeSee Development Workflow Setup

**Create and maintain the development workflow system for SeeSee.**

**On first run, save this prompt to `.claude/commands/setup-dev-guide.md`.**
...
- **Name:** SeeSee
- **Purpose:** Lightweight, self-hosted sent email log aggregator with search UI
- **Website:** seesee.email
- **Tagline:** See what your apps sent.
- **Type:** Self-hosted web application (API + SMTP server + Web UI)
...
| ORM/DB Access | aiosqlite (async SQLite) |
| Settings | pydantic-settings |
| Templates | Jinja2 |
| UI Styling | Tailwind CSS (CDN, no build step) |
| UI Interactivity | Alpine.js (CDN, no build step) |
| Auth | API keys (REST), SMTP credentials (SMTP), session cookies (UI) |
| Container | Docker (single container) |
| CI/CD | GitHub Actions → GHCR |
| Docs Site | Astro Starlight → GitHub Pages |
| Package Manager | pip / pyproject.toml |
| Test Framework | pytest + pytest-asyncio + httpx (for FastAPI test client) |

### Architecture Summary

SeeSee is a single-container application with three ingest/interface layers:

1. **REST API** (port 8080) — Apps POST email log entries via HTTP
2. **SMTP Ingest** (port 2525) — Apps send via SMTP; SeeSee captures and optionally relays
3. **Web UI** (port 8080) — Admin dashboard for searching and viewing logged emails

All three share a single SQLite database. The UI is server-rendered (Jinja2), not an SPA.

---

## Create `.claude/commands/dev.md`

Generate the following development workflow guide, filling in actual values as the project develops:
...
```bash
git clone https://github.com/[owner]/seesee-email.git
cd seesee-email
python -m venv .venv
...
**Version:** [current] (Semantic Versioning)
...
├── docs/                    # Astro Starlight docs site (independent, not required to run SeeSee)
...
- Code: Google-style docstrings on public functions/classes
- API: Auto-generated OpenAPI at `/docs` (FastAPI handles this)
- User docs: Markdown in `docs/src/content/docs/` (Starlight)
- Dev docs: This file + ROADMAP.md + CHANGELOG.md
...
Review: ROADMAP.md current phase → CHANGELOG.md Unreleased section → this file's priority list.

### During Development

1. Write/update tests alongside code
2. Run frequently: `pytest -x` and `ruff check .`
3. Test the UI manually in browser (dark + light mode)
...
3. Bump version in `seesee/__init__.py` + `pyproject.toml`
4. Update `CHANGELOG.md` Unreleased section
5. Update `ROADMAP.md` if phase items completed
6. Commit, push, merge to main
7. Delete feature branch
8. Summarize what was done + suggest next steps
...
Create with the phase structure defined above. Each phase should have clear objectives, items, and status indicators.
...
### Added
- Project specification and documentation structure
...
Include:
- "SeeSee is early-stage software — contributions welcome"
- Development setup (clone, venv, pip install -e .[dev])
- Running tests (pytest)
- Code style (Ruff, PEP 8)
- Git workflow (feature branches, conventional commits)
- PR process
- Where to find work items (ROADMAP.md, issues)

### `.env.example`

Document every `SEESEE_*` environment variable with descriptions and defaults. See the audit prompt for the full list.
...
- [ ] `.claude/commands/dev.md` created with all SeeSee-specific details
- [ ] `.claude/commands/setup-dev-guide.md` saved (this prompt)
- [ ] `.claude/commands/audit.md` saved (audit prompt)
- [ ] `ROADMAP.md` has phased plan with status indicators
- [ ] `CHANGELOG.md` initialized with Keep a Changelog format
- [ ] `CONTRIBUTING.md` covers setup, standards, workflow
- [ ] `.env.example` documents all SEESEE_* variables
- [ ] `.gitignore` covers Python, SQLite, docs, IDE, OS files
- [ ] `README.md` has quick start, badges, links
- [ ] All commands in dev.md are accurate and tested
- [ ] Version number is consistent across __init__.py and pyproject.toml
...
1. **Files created/updated** with brief descriptions
2. **Current phase** and recommended first task
3. **Quick start commands** verified working
4. **Items needing human input** (GitHub org name, domain DNS, etc.)
